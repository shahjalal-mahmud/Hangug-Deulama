...
# Hangug Deulama — Frontend Architecture & Defense Guide
...
This document is anchored for `@see` cross-references from frontend source
files. Anchor ids below are permanent; heading wording may change later
without breaking links.

---

<a id="sec-fe-overview"></a>
...
<a id="sec-api-client"></a>

## 2. API Client — Axios Setup, Interceptors, 401 Bus, Error Envelope

<a id="sec-api-client-config"></a>
...
<a id="sec-api-request-interceptor"></a>
...
<a id="sec-api-response-interceptor"></a>

### 2.4 Response Interceptor
...
<a id="sec-api-error-normalization"></a>
...
- The shape matches the documented error envelope
  ...
  per [Error](docs/API.md#sec-response-error).

---

<a id="sec-endpoint-mapping"></a>

## 3. Endpoint Mapping (Frontend ↔ Backend API)

All paths below are relative to `apiClient`'s baseURL
(`${API_BASE_URL}/api`). Quotes are from `docs/API.md`.

<a id="sec-endpoint-auth"></a>

### 3.1 Auth

| Frontend (verified)                                                | docs/api.md section                                      | Auth                                                                              | Notes                                                                                                                                                                                       |
| ------------------------------------------------------------------ | -------------------------------------------------------- | --------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `register(payload)` — `POST /auth/register` (`src/api/auth.js:10`) | [POST /api/auth/register](docs/API.md#sec-auth-register) | Not required                                                                      | Response `201` per docs/api.md; payload `{ full_name, email, password, password_confirmation }` (per [docs/api.md §"POST /api/auth/register"](docs/API.md#sec-auth-register) Request Body). |
| `login(payload)` — `POST /auth/login` (`src/api/auth.js:15`)       | [POST /api/auth/login](docs/API.md#sec-auth-login)       | Not required                                                                      | Returns `{ data: { user, token } }` per [docs/api.md §"POST /api/auth/login"](docs/API.md#sec-auth-login) Success Response.                                                                 |
| `me()` — `GET /me` (`src/api/auth.js:20`)                          | [GET /api/me](docs/API.md#sec-auth-me)                   | **Required** Bearer JWT per [docs/api.md §"GET /api/me"](docs/API.md#sec-auth-me) | Used as the auto-login smoke test on app boot. Returns `{ data: { user_id, full_name, email, profile_image, created_at } }` per docs/api.md.                                                |

Public + JWT-required statuses:

- per [HTTP Status Codes](docs/API.md#sec-http-status-codes): `401` for missing/expired/malformed JWT or wrong credentials on `/auth/login`; `422` for validation; `404` (`auth.user_not_found`) when the token resolves but the user row is gone.

<a id="sec-endpoint-dramas"></a>

### 3.2 Dramas (`src/api/dramas.js`)
...
- `listDramas({ page, limit, sort, order })` → `GET /dramas` (`dramas.js:25`).
  ...
  - per [GET /api/dramas](docs/API.md#sec-dramas-list): defaults `page=1`, `limit=20`,
  ...
  - per [GET /api/dramas](docs/API.md#sec-dramas-list) Error: `422` when page/limit/sort/order bad.
- `getDrama(id)` → `GET /dramas/{id}` (`dramas.js:33`). per docs/api.md: `422`
  on bad id, `404` if drama missing.

<a id="sec-endpoint-favorites"></a>

### 3.3 Favorites (`src/api/favorites.js`)
...
<a id="sec-endpoint-watch-later"></a>

### 3.4 Watch Later (`src/api/watchLater.js`)
...
<a id="sec-endpoint-watched"></a>

### 3.5 Watched (`src/api/watched.js`)

- `markWatched` → `POST /watched` (`watched.js:6`).
- `listWatched` → `GET /watched` (`watched.js:9`).
- per [POST /api/watched](docs/API.md#sec-watched-add): no `DELETE` endpoint exists by design
  (write-once). Same `201/409/404/422` lifecycle as siblings.

<a id="sec-endpoint-swipe"></a>

### 3.6 Swipe (`src/api/swipe.js`)

- `recordSwipe(dramaId, swipeType)` → `POST /swipe` (`swipe.js:7`) with
  `{ drama_id, swipe_type }`.
- per [POST /api/swipe](docs/API.md#sec-swipe-endpoint): `201 Created` on the first swipe for
...
<a id="sec-endpoint-profile"></a>

### 3.7 Profile (`src/api/profile.js`)

- `getProfile` → `GET /profile` (`profile.js:9`). per [GET /api/profile](docs/API.md#sec-profile-get):
...
- `getGenreStatistics` → `GET /profile/genre-statistics` (`profile.js:12`).
  per [GET /api/profile/genre-statistics](docs/API.md#sec-genre-statistics-endpoint): returns
...
- `updateProfile({ name, currentPassword, newPassword, confirmPassword, image })`
  ...
  - **With image** → `FormData` body. Per `profile.js:40` (in-code note):
    ...
    malformed body … silently fails."_ This mirrors [PUT /api/profile](docs/API.md#sec-profile-update)
...
<a id="sec-endpoint-recommendations"></a>

### 3.8 Recommendations (`src/api/recommendations.js`)

- `getRecommendations` → `GET /recommendations` (`recommendations.js:6`).
- per [GET /api/recommendations](docs/API.md#sec-recommendations-endpoint): returns up to 10 dramas with
  `{ recommendations, count, is_personalized, fallback }`. Internal
  scores are never exposed.

<a id="sec-endpoint-health"></a>

### 3.9 Health (`src/api/health.js`)

- `checkHealth` → `GET /health` (`health.js:6`). Public liveness probe;
  response `{ status: 'ok', time, app }` per [GET /api/health](docs/API.md#sec-health-endpoint).

---

<a id="sec-state-management"></a>

## 4. State Management

<a id="sec-auth-context"></a>

### 4.1 AuthContext — Token Bootstrap, Persistence, Optimistic isAuthenticated
...
- `login()` / `register()` (`AuthContext.jsx:112`, `:126`) share
  ...
  shape `{ data: { user, token } }` per [POST /api/auth/login](docs/API.md#sec-auth-login).)
...
- **Mutations** (all optimistic):
  ...
  - **`watchDrama(id)`** (`:164`): guarded by
    ...
    returns `409` for duplicates per [POST /api/watched](docs/API.md#sec-watched-add);
...
<a id="sec-recommendation-scoring-docs"></a>

## 5. Recommendation Scoring & Cold-Start (docs-sourced, not code-verified)
...
<a id="sec-known-discrepancies"></a>

## 6. Known Documentation Discrepancies
...
1. **`users.profile_image` vs `users.avatar_url`.** PROJECT.md §12.1
   ...
   **`profile_image`** (e.g. `docs/api.md:224`, `:394`, `:458`, `:507`).
   [GET /api/profile](docs/API.md#sec-profile-get) uses **`image`**.
...
## 7. Frequently Asked Integration Questions
...
5. **"How is the bootstrap session restored?"** → `AuthContext` reads
   ...
   session.
6. **"How long does the JWT live?"** → per [Token claims](docs/API.md#sec-auth-token-claims):
...
13. **"What happens if the user doubles on a swipe?"** → per [POST /api/swipe](docs/API.md#sec-swipe-endpoint)
    "Re-swiping the same type returns `200 OK` rather
...
<a id="sec-fe-anchor-index"></a>

## Anchor Index

| Anchor id                         | Heading                                                                    |
| --------------------------------- | -------------------------------------------------------------------------- |
| `sec-fe-top`                      | Hangug Deulama — Frontend Architecture & Defense Guide                     |
| `sec-fe-overview`                 | 1. Architecture in 30 Seconds                                              |
| `sec-api-client`                  | 2. API Client — Axios Setup, Interceptors, 401 Bus, Error Envelope         |
| `sec-api-client-config`           | 2.1 Client & Config                                                        |
| `sec-api-request-interceptor`     | 2.2 Request Interceptor — Bearer Token                                     |
| `sec-api-401-bus`                 | 2.3 401 Event Bus                                                          |
| `sec-api-response-interceptor`    | 2.4 Response Interceptor                                                   |
| `sec-api-error-normalization`     | 2.5 Error Envelope Normalization                                           |
| `sec-endpoint-mapping`            | 3. Endpoint Mapping (Frontend ↔ Backend API)                               |
| `sec-endpoint-auth`               | 3.1 Auth                                                                   |
| `sec-endpoint-dramas`             | 3.2 Dramas                                                                 |
| `sec-endpoint-favorites`          | 3.3 Favorites                                                              |
| `sec-endpoint-watch-later`        | 3.4 Watch Later                                                            |
| `sec-endpoint-watched`            | 3.5 Watched                                                                |
| `sec-endpoint-swipe`              | 3.6 Swipe                                                                  |
| `sec-endpoint-profile`            | 3.7 Profile                                                                |
| `sec-endpoint-recommendations`    | 3.8 Recommendations                                                        |
| `sec-endpoint-health`             | 3.9 Health                                                                 |
| `sec-state-management`            | 4. State Management                                                        |
| `sec-auth-context`                | 4.1 AuthContext — Token Bootstrap, Persistence, Optimistic isAuthenticated |
| `sec-drama-context`               | 4.2 DramaContext — Optimistic Mutations + Anonymous Storage                |
| `sec-anonymous-sync-gap`          | 4.3 Login-Sync vs. Anonymous-Local — What the Code Actually Does           |
| `sec-recommendation-scoring-docs` | 5. Recommendation Scoring & Cold-Start (docs-sourced, not code-verified)   |
| `sec-known-discrepancies`         | 6. Known Documentation Discrepancies                                       |
| `sec-faq`                         | 7. Frequently Asked Integration Questions                                  |
| `sec-fe-glossary`                 | 8. Glossary                                                                |
| `sec-fe-anchor-index`             | Anchor Index                                                               |
