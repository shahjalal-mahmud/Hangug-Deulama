...
const DramaDetails = () => {
  ...
  const similarDramas = dramas
    .filter(d => 
      d.id !== drama.id && 
      d.genre.some(g => drama.genre.includes(g))
    )
    .slice(0, 4);
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div className="md:col-span-2 space-y-4">
          ...
          <div className="flex flex-wrap gap-2 items-center">
            ...
            <span className="badge badge-ghost">📅 {drama.year}</span>
            <span className="badge badge-secondary">⭐ {drama.rating}/5.0</span>
          </div>
      ...
      {similarDramas.length > 0 && (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {similarDramas.map((d) => (
              <div key={d.id} className="drama-card">
                <figure>
                  <img 
                    src={d.poster} 
                    alt={d.title}
                ...
                <div className="card-body">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {d.genre.slice(0, 2).map((g, idx) => (
                      <GenreBadge key={idx} genre={g} />
                    ))}
                  </div>
                  <div className="card-actions justify-end">
                    <Link to={`/drama/${d.id}`} className="btn btn-primary btn-xs">
                      View
                    </Link>
                  </div>
                </div>
